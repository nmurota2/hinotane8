...
⚠️ 実測の結果、この順位づけには選別能力が無いことが分かっている
（順位を乱数にした対照群と差が出ず、情報係数も実質ゼロ）。
...
順位づけ: 7-1 モメンタム（約 7 か月の上昇率から直近 1 か月を除いたもの）
"""
...
class TrendStrategy(Strategy):
    ...
    def evaluate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 3 件しか取れないので、「何を選ぶか」が成績をほぼ決める……はずだった。
        # 実測では乱数と差が出なかった。この順位づけは効いていない。
