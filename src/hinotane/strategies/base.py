...
class Strategy(ABC):
    ...
    #: 決済判定にどの値を使うか。"low" は日中安値、"close" は終値。
    #:
    #: ⚠️ ここは見た目より重い。日中安値で判定すると、ヒゲで一瞬触れただけでも
    #: 決済が確定する。しかも本番は引け後に判定して翌朝の寄りで売るので、
    #: 「逆指値と同じ頻度で発動するのに、逆指値の価格はもらえない」という
    #: 一方的に不利な組み合わせになる。終値判定なら発動頻度が減り、
    #: 実効的な損切り幅が広がって回転数が落ちる。
    exit_trigger: str = "low"

    #: 損切りで decision するか。False なら損切り価格に触れても決済しない。
    #: **検証用の対照群のためだけにある。** 損切りの寄与を測るには
    #: 「損切りだけ無い設定」と比べる必要があるが、損切り価格を遠くに置く
    #: やり方では数量計算のリスク幅が巨大になり、単元株に届かず 1 件も
    #: 建たなくなる（実際に起きた）。数量と R 倍率は今までどおり損切り幅で
    #: 決めたまま、決済判定だけを止める。
    use_stop_exit: bool = True
